...
read_count_output_modified <- function(dir, name, type = 'total', batch = FALSE, tcc = FALSE) {
    dir <- normalizePath(dir, mustWork = TRUE)
    if (type == '') {
        m <- readMM(paste0(dir, "/", name, ".mtx"))
    } else {
        m <- readMM(paste0(dir, "/", name, ".", type, ".mtx"))
    }
    m <- Matrix::t(m)
    m <- as(m, "dgCMatrix")
    # The matrix read has cells in rows
    ge <- if (tcc) ".ec.txt" else ".genes.names.txt"
...
sort_clusters_by_agreement <- function(clus_df_gather, stable_column = "Seurat", reordered_column = "Scanpy") {
    for (n in 1:2) {
        ...
        clus_df_gather[[reordered_column]] <- as.numeric(clus_df_gather[[reordered_column]])
        ...
        clus_df_gather[[reordered_column]] <- as.factor(clus_df_gather[[reordered_column]])
        
        clus_df_gather$y<- as.numeric(clus_df_gather$y)
        clus_df_gather$y[((half_rows + 1):nrow(clus_df_gather))] <- subset_data[[reordered_column]]
        clus_df_gather$y <- as.factor(clus_df_gather$y)
